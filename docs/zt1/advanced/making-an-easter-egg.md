...
You can use this template script to create any easter egg you want. You can make a guest say something funny, make an animal do a trick, or even make a building explode. The possibilities are endless. Of course, your mileage may vary depending on what is currently supported in EMU API, but it's worth experimenting with with all of the functions detailed in the [EMU API documentation](../reference/emu/api/index.md). If you need a primer on Lua scripting to get the logic just right, I recommend reading the [Scripting in Zoo Tycoon 1](./lua-scripting-in-zt1.md) guide.